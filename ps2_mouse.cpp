...
void PS2Mouse::write_byte(uint8_t data) {
  char i;
  char parity = 1;
  pull_high(g_data_pin);
  pull_high(g_clk_pin);
...
void PS2Mouse::begin(uint8_t clk_pin, uint8_t data_pin) {
  ...
  read_byte();       // ignore
  delay(20);
  write_byte(0xf4);  // enable data reporting
  read_byte();       // read ack
